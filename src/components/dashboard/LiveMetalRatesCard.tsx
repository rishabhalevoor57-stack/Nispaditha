...
export function LiveMetalRatesCard() {
  ...
  const fetchRate = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('silver_rate_per_gram, updated_at')
        .limit(1)
        .single();
  ...
  const fetchHistory = async () => {
    try {
      const { data, error } = await supabase
        .from('rate_history')
        .select('id, silver_rate_per_gram, created_at')
  ...
  const handleEdit = () => {
    if (rate) {
      setEditSilverRate(rate.silver_rate_per_gram.toString());
    }
    setIsEditing(true);
  };
      ...
      <Card className="border-l-4 border-l-primary">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1">
            <p className="text-xs text-muted-foreground">Silver Rate Per Gram</p>
            <p className="text-2xl font-bold text-primary">
              {formatCurrency(rate?.silver_rate_per_gram || 0)}/g
            </p>
          </div>

          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <Clock className="w-3 h-3" />
            Last updated: {rate?.updated_at ? format(new Date(rate.updated_at), 'dd MMM yyyy, hh:mm a') : '-'}
          </div>
